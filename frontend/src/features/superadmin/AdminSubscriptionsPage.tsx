        ...
        {ov && (
          <div className="flex-shrink-0 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-px bg-zinc-800 border-b border-zinc-800">
            {[
              ...
              { label: 'Monthly Revenue', value: `$${Number(ov.monthly_revenue).toFixed(0)}` },
            ].map(stat => (
          ...
          {tab === 'proofs' && (
            <div className="max-w-4xl">
              <p className="text-xs text-zinc-500 mb-4">
                Payment proofs are submitted per-tenant. Navigate to a tenant subscription to review their proofs, or use the list below.
              </p>
              <p className="text-sm text-zinc-600 text-center py-8">
                Select a subscription from the Subscriptions tab to review its payment proofs.
              </p>
            </div>
          )}
        </div>
